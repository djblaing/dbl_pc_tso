...
#object name is just the name of the parent directory, which is the same as the name of the h5 file without the .h5 extension. 
object_name = Path(__file__).parent.stem
...
def example_1_broadband():
    ...
    config_path = Path(__file__).parent / f"{object_name}_config_broadband.toml"
...
def example_2_nbins():
    ...
    config_path = Path(__file__).parent / f"{object_name}_config_nbin.toml"
    
    if not config_path.exists():
        print(f"Config file not found: {config_path}")
        print(f"Please edit {object_name}_config_nbin.toml with your data path first.")
        return
...
def example_3_custom_regions():
    ...
    config_path = Path(__file__).parent / f"{object_name}_config_custom.toml"
    
    if not config_path.exists():
        print(f"Config file not found: {config_path}")
        print(f"Please edit {object_name}_config_custom.toml with your data path first.")
        return
